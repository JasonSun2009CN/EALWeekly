(function(){
  const translations = {
    en: {
      ...
      title: 'Title', content: 'Content', filterWeek: 'Filter by week', filterDate: 'Filter by date',
      ...
      noResults: 'No results', searchUnavailable: 'Search index not available.'
    },
    zh: {
  ...
  fetch(searchIndexUrl).then(r=>r.json()).then(data=>{
    ...
    resultsEl.innerHTML = `<p class="muted">${translate('searchUnavailable')}</p>`;
    console.error(err);
  });
